        ...
        for j in range(1, hyp_lgt):
            if ref[i - 1] == hyp[j - 1]:
                ...
                if min_cost < costs[i, j]:
                    costs[i, j] = min_cost
                    backtrace[i, j] = [sub_cost, ins_cost, del_cost].index(
                        costs[i, j]
                    ) + 1
...
def sent_evaluation(**kwargs):
    if "conv_prediction" in kwargs.keys():
        ret1 = get_wer_delsubins(
            kwargs["gt"],
            kwargs["conv_prediction"],
            merge_same=kwargs["merge_same"],
            penalty=kwargs["penalty"],
        )
        ret2 = get_wer_delsubins(
            kwargs["gt"],
            kwargs["lstm_prediction"],
            merge_same=kwargs["merge_same"],
            penalty=kwargs["penalty"],
        )
        new_gt = get_wer_delsubins(
            ret1[0], ret2[0], merge_same=kwargs["merge_same"], penalty=kwargs["penalty"]
        )[0]
    ...
    gt, lstm_pred = get_wer_delsubins(
        kwargs["gt"],
        kwargs["lstm_prediction"],
        merge_same=kwargs["merge_same"],
        penalty=kwargs["penalty"],
...
if __name__ == "__main__":
    wer_calculation("phoenix2014-groundtruth-dev.stm", "out.output-hypothesis-dev.ctm")
