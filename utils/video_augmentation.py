...
class WERAugment(object):
    ...
    def delete(ind, label, binfo):
        ...
        del label[del_wd]
        binfo = [i for i in binfo[:del_wd]] + [
            i - duration for i in binfo[del_wd + 1 :]
        ]
    ...
    def insert(ind, label, binfo):
        ...
        binfo = (
            binfo[:ins_lab_pos]
            + [binfo[ins_lab_pos - 1] + duration]
            + [i + duration for i in binfo[ins_lab_pos:]]
        )
        return ind, label, binfo
    ...
    def substitute(ind, label, binfo):
        ...
        ind = (
            ind[: binfo[tar_wd]]
            + ind[binfo[sub_wd] : binfo[sub_wd + 1]]
            + ind[binfo[tar_wd + 1] :]
        )
        label[tar_wd] = label[sub_wd]
        delta_duration = (
            binfo[sub_wd + 1] - binfo[sub_wd] - (binfo[tar_wd + 1] - binfo[tar_wd])
        )
...
class RandomResize(object):
    ...
    def __call__(self, clip):
        ...
        if isinstance(clip[0], np.ndarray):
            return [
                scipy.misc.imresize(img, size=(new_h, new_w), interp=self.interpolation)
                for img in clip
            ]
        elif isinstance(clip[0], PIL.Image.Image):
            return [
