    ...
    def criterion_calculation(self, ret_dict, label, label_lgt, phase):
        ...
        for k, weight in self.loss_weights.items():
            if k == "ConvCTC":
                ...
                contrast_hn_acc = torch.eq(torch.argmax(hn_pred[:B], dim=1), target).float().mean()
                contrast_en_acc = torch.eq(torch.argmax(en_pred[B:], dim=1), target).float().mean()
                ...
                hn_loss = self.loss["CE"](hn_pred, target)
                en_loss = self.loss["CE"](en_pred, target)
                hn_acc = torch.eq(torch.argmax(hn_pred[:B], dim=1), target).float().mean()
                en_acc = torch.eq(torch.argmax(en_pred[B:], dim=1), target).float().mean()
                loss_kv[f"{phase}/Loss/{k}-hn_loss"] = hn_loss.item()
                loss_kv[f"{phase}/Loss/{k}-en_loss"] = en_loss.item()
